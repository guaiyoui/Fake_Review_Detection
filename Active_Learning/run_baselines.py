...
class run_wrapper():
    def __init__(self, dataset, normalization, cuda):
        if dataset in ['spammer']:
            ...
            self.labels = torch.from_numpy(labels_data[:, 2]).cuda()

            training_data = np.loadtxt("../Spammer-ISR-Initial-Exp/ISR-spammer-detection/Data/Training_Testing/5percent/train_4.csv", delimiter=' ', dtype=int)
            testing_data = np.loadtxt("../Spammer-ISR-Initial-Exp/ISR-spammer-detection/Data/Training_Testing/5percent/test_4.csv", delimiter=' ', dtype=int)
        ...
        # print(f'self.idx_test is {self.idx_test}, self.idx_non_test is {self.idx_non_test}')
