...
do
    ...
    echo "Running baseline with dataset $weight_loss, wuth global sampling"
    CUDA_VISIBLE_DEVICES=2 nohup python run_ComGA.py \
        --dataset amazon \
        --model GCN \
        --epoch 300 \
        ...
        --sample_global \
        >> ./logs/loss_${weight_loss}_global.txt 2>&1 &
done
